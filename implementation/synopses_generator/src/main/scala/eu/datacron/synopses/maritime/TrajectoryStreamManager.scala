...
import eu.datacron.synopses.ShipInfo
...
object TrajectoryStreamManager{
  ...
  config = config.fromFile(configProperties)    //OPTION #1: from a properties array already loaded
  ...
  def main(args: Array[String]) {
    ...
    val in_file: String = args(1)
    val crit_file: String  = args(2)
    val loc_file: String  = args(3)
    val not_file: String  = args(4)
    var print_noisy_messages: Boolean = true;
    if(args.length >= 6)
      print_noisy_messages = args(5) != "false"
    val tmp = new File(not_file)
    tmp.delete()
    // val notFile: FileWriter = new FileWriter(not_file)
    ...
    // System.out.println("TYPES: " + shipInfo.toString())
      ...
      def filter(pos: critical_point): Boolean = {
        ...
        if (config.BBOX.contains(pos.getLongitude, pos.getLatitude)) {         //Only keep raw positions within the specified area of monitoring
          if (pos.getTimestamp > lastInputTimestamp) {
            ...
            if (lastInputTimestamp - lastCleanupTimestamp > config.PARAMS.HISTORY_PERIOD(shipInfo.getType(pos.getId.toString), pos) * 1000L) {
              for (vec <- objStates)  {          //Applied to all objects regardless
                ...
                if ((vec._2.objHistory.length > 0) && (vec._2.objHistory.last.getTimestamp < (lastInputTimestamp - config.PARAMS.HISTORY_PERIOD(shipInfo.getType(pos.getId.toString),pos) * 1000L))) {
                  vec._2.purgeState()
          ...
          if (!objStates.contains(pos.getId)) {                   //This is the first location after a communication GAP, ...
            ...
            objStates(pos.getId).initState(pos, config.PARAMS.HISTORY_PERIOD(shipInfo.getType(pos.getId.toString),pos), config.PARAMS.BUFFER_SIZE(shipInfo.getType(pos.getId.toString),pos)) //Queue of recent locations is initialized with the current one
          }
          return true
        }
    ...
    val criticalPointStream: DataStream[critical_point] = positionalStream
      ...
      .reduce { (prevLoc: critical_point, curLoc: critical_point) => forwardCheckMobilityFeatures(prevLoc, curLoc, not_file) }   //FORWARD check: characterizes the CURRENT location
      .setParallelism(1)
    ...
    turningPointStream
      .filter(_.getAnnotation.getNoise == print_noisy_messages)
      .writeAsText(loc_file)
    ...
    turningPointStream.filter(new FilterFunction[critical_point]() {
      ...
      .writeAsText(crit_file)      //OUTPUT resulting critical points as a Kafka stream at that specific topic


    val jobResult = env.execute("Maritime Trajectory Stream Manager " + args(0))
  }
  ...
  def parseLocation(msg : String): (critical_point) = {
    ...
    new critical_point(tokens(0).toLong, tokens(1).toString, tokens(2).toDouble, tokens(3).toDouble, null, 0.0F, 0.0F, 0.0F, 0L, tokens(6).toString, System.currentTimeMillis(), -1.0F, -1.0F)
  }
  ...
  def eliminateNoise(oldLoc: critical_point, newLoc: critical_point): Boolean = {
    ...
    if ((newLoc.getSpeed < 0.0F) || (newLoc.getSpeed >= config.PARAMS.MAX_SPEED_THRESHOLD(shipInfo.getType(newLoc.getId.toString),newLoc))) {
      isNoise = true
    }
    ...
    else if (Math.abs(MobilityChecker.getRateOfChangeKnots(oldLoc, newLoc)) >= config.PARAMS.MAX_RATE_OF_CHANGE(shipInfo.getType(newLoc.getId.toString),newLoc)) {
    ...
    else if ((newLoc.getSpeed > config.PARAMS.LOW_SPEED_THRESHOLD(shipInfo.getType(newLoc.getId.toString),newLoc)) && (MobilityChecker.getRateOfTurn(newLoc, oldLoc) >= config.PARAMS.MAX_RATE_OF_TURN(shipInfo.getType(newLoc.getId.toString),newLoc))) {
      isNoise = true                     //Mark this location as noise
    }
  ...
  def backwardCheckMobilityFeatures(oldLoc: critical_point, newLoc: critical_point): critical_point = {
    ...
    // Calculalate and store the heading difference used for critical turning points.
    oldLoc.setHeadingDiff(
      Math.max(
        MobilityChecker.angleDifference(newLoc.getHeading, curState.getMeanHeading),
        Math.abs(curState.getCumulativeHeading)
      )
    )
    ...
    if (!curState.isStopped) {
      ...
      if (oldLoc.getHeadingDiff() > config.PARAMS.ANGLE_THRESHOLD(shipInfo.getType(newLoc.getId.toString),newLoc)) {
  ...
  def forwardCheckMobilityFeatures(prevLoc: critical_point, newLoc: critical_point, not_file: String): critical_point = {
    ...
    if (newLoc.getTimeElapsed > 1000.0F * config.PARAMS.GAP_PERIOD(shipInfo.getType(newLoc.getId.toString),newLoc)) {
      ...
      val tmp: FileWriter = new FileWriter(not_file, true)
      try {
        tmp.write(oldLoc.toString + "\n")
      }
      finally tmp.close() 
    ...
    if (newLoc.getAnnotation.getNoise == true) {
      ...
      if(!objStates.contains(newLoc.getId))
        objStates += (newLoc.getId -> new ObjectState())
///// This threw exception
    ...
    newLoc.setPercentalSpeedChange(
      Math.abs((newLoc.getSpeed - mean_speed) / mean_speed)
    )

    //CAUTION! Both check conditions concerning STOP cannot hold simultaneously!
    if ((newLoc.getSpeed < config.PARAMS.NO_SPEED_THRESHOLD(shipInfo.getType(newLoc.getId.toString),newLoc)) && !curState.isStopped) {
    ...
    else if (((newLoc.getSpeed >= config.PARAMS.NO_SPEED_THRESHOLD(shipInfo.getType(newLoc.getId.toString),newLoc)) || (newLoc.getDistance >= config.PARAMS.DISTANCE_THRESHOLD(shipInfo.getType(newLoc.getId.toString),newLoc))) && curState.isStopped) {
    ...
    if (!curState.isStopped) {
      ...
      if ((newLoc.getPercentalSpeedChange() > config.PARAMS.SPEED_RATIO(shipInfo.getType(newLoc.getId.toString),newLoc)) && !curState.hasChangedSpeed) {
      ...
      if ((newLoc.getSpeed <= config.PARAMS.LOW_SPEED_THRESHOLD(shipInfo.getType(newLoc.getId.toString),newLoc)) && (oldLoc.getSpeed > config.PARAMS.LOW_SPEED_THRESHOLD(shipInfo.getType(newLoc.getId.toString),newLoc)) && !curState.inSlowMotion) {
