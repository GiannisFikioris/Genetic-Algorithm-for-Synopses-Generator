...
cd ~/infore/kafka_2.11-0.10.2.1/bin
...
~/infore/kafka_2.11-0.10.2.1/bin/kafka-topics.sh --create --zookeeper localhost:2181 --replication-factor 1 --partitions 1 --topic ais-messages
~/infore/kafka_2.11-0.10.2.1/bin/kafka-topics.sh --create --zookeeper localhost:2181 --replication-factor 1 --partitions 3 --topic ais-locations
~/infore/kafka_2.11-0.10.2.1/bin/kafka-topics.sh --create --zookeeper localhost:2181 --replication-factor 1 --partitions 3 --topic ais-critical-points
~/infore/kafka_2.11-0.10.2.1/bin/kafka-topics.sh --create --zookeeper localhost:2181 --replication-factor 1 --partitions 3 --topic ais-notifications


-- Drop existing topics like this:
-- IMPORTANT: change <MY_INPUT_TOPIC> <MY_OUTPUT_TOPIC> and into topic names of your choice, and <MY_HOST> with the DNS IP address of your localhost:

~/infore/kafka_2.11-0.10.2.1/bin/kafka-topics.sh --delete --zookeeper localhost:2181 --topic ais-messages
~/infore/kafka_2.11-0.10.2.1/bin/kafka-topics.sh --delete --zookeeper localhost:2181 --topic ais-locations
~/infore/kafka_2.11-0.10.2.1/bin/kafka-topics.sh --delete --zookeeper localhost:2181 --topic ais-critical-points
~/infore/kafka_2.11-0.10.2.1/bin/kafka-topics.sh --delete --zookeeper localhost:2181 --topic ais-notifications
...
* Assuming that Avro tools (avro-1.8.1.jar; avro-tools-1.8.1.jar; jackson-core-asl-1.9.13.jar; jackson-mapper-asl-1.9.13.jar) are installed in: ~/infore/flink-0.10.2/lib
...
* Assuming that Avro schema specifications (files *.avsc) are created in: ~/infore/datacron/implementation/trajectory_synopses/src/main/resources

* Build a POJO (Plain Old Java Object) from a specification in file 'critical_point.avsc': 

java -jar ~/infore/flink-0.10.2/lib/avro-tools-1.8.1.jar compile schema ~/infore/datacron/implementation/trajectory_synopses/src/main/resources/critical_point.avsc /trajectory_synopses/src/main/resources
...
~/infore/flink-0.10.2/bin/flink run -c eu.datacron.synopses.maritime.StreamOriginalRateSimulator ~/infore/datacron/implementation/trajectory_synopses/target/datacron_trajectory_synopses-0.7.jar ais-messages localhost:9092 0

-- AVIATION dataset:

~/infore/flink-0.10.2/bin/flink run -c eu.datacron.synopses.aviation.StreamOriginalRateSimulator ~/infore/datacron/implementation/trajectory_synopses/target/datacron_trajectory_synopses-0.7.jar adsb_messages localhost:9092 0
...
~/infore/kafka_2.11-0.10.2.1/bin/kafka-console-consumer.sh --bootstrap-server localhost:9092 --topic ais-critical-points

-- or from the beginning (how many data is retained depends on Kafka retention time parameter):

~/infore/kafka_2.11-0.10.2.1/bin/kafka-console-consumer.sh --bootstrap-server <MY_HOST>:9092 --topic <MY_OUTPUT_TOPIC> --from-beginning 

-- Results may be written to a JSON file:

~/infore/kafka_2.11-0.10.2.1/bin/kafka-console-consumer.sh --bootstrap-server <MY_HOST>:9092 --topic <MY_OUTPUT_TOPIC> --from-beginning > ~/infore/datacron/data/output/test1.json


-- List all Kafka topics:

~/infore/confluent-3.2.0/bin/kafka-topics --zookeeper <MY_HOST>:2181 --list
...
~/infore/flink-0.10.2/bin/flink run -c eu.datacron.synopses.maritime.TrajectoryStreamManager ~/infore/datacron/implementation/trajectory_synopses/target/datacron_trajectory_synopses-0.7.jar


-- AVIATION:

~/infore/flink-0.10.2/bin/flink run -c eu.datacron.synopses.aviation.TrajectoryStreamManager ~/infore/datacron/implementation/trajectory_synopses/target/datacron_trajectory_synopses-0.7.jar
